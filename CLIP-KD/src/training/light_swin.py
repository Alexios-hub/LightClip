...
class ShapedWindowAttention(nn.Module):
    ...
    def __init__(self, dim, num_heads, head_dim=None, window_size=7, qkv_bias=True, attn_drop=0., proj_drop=0.):
        ...
        self.qk = nn.Linear(dim, attn_dim * 2, bias=qkv_bias)
    ...
    def forward(self, x, mask: Optional[torch.Tensor] = None):
        ...
        """
        B_, N, C = x.shape
        qk = self.qk(x).reshape(B_, N, 2, self.num_heads, -1).permute(2, 0, 3, 1, 4)
        q, k = qk.unbind(0)  # make torchscript happy (cannot use tensor as tuple)
        v = x.reshape(B_, N, self.num_heads,-1).permute(0, 2, 1, 3)
...
class ParallelSwinTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.attn = ShapedWindowAttention(
    ...
    def forward(self, x):
        ...
        if self.shift_size > 0:
            x_att = torch.roll(shifted_x, shifts=(self.shift_size, self.shift_size), dims=(1, 2))
        else:
            x_att = shifted_x
        x_att = x_att.view(B, H * W, C)

        x_mlp = self.drop_path(self.mlp(x.view(B, H * W, C)))
        x = x_att + x_mlp
...
class LightSwinTransformer(nn.Module):
    r""" Swin Transformer
...
def _create_lightweight_swin_transformer(variant, pretrained=False, **kwargs):
    model = build_model_with_cfg(
        LightSwinTransformer, variant, pretrained,
...
@register_model
def light_swin_tiny_patch4_window7_224(pretrained=False, **kwargs):
    """ Swin-T @ 224x224, trained ImageNet-1k
    """
    model_kwargs = dict(
        patch_size=4, window_size=7, embed_dim=96, depths=(2, 2, 6, 2), num_heads=(3, 6, 12, 24), **kwargs)
    return _create_lightweight_swin_transformer('light_swin_tiny_patch4_window7_224', pretrained=pretrained, **model_kwargs)
